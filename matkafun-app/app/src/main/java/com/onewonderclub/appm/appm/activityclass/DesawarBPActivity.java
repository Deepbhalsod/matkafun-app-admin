...
import com.onewonderclub.appm.appm.adapterclass.DesawarBAdapter;
import com.onewonderclub.appm.appm.apiclass.ApiClass;
import com.onewonderclub.appm.appm.responseclass.DataDesawarBid;
import com.onewonderclub.appm.appm.responseclass.DataMain;
import com.onewonderclub.appm.appm.shareprefclass.MBSFragment;
...
import com.onewonderclub.market.adapterclass.JodiSuggestionAdapter;
...
public class DesawarBPActivity extends AppCompatActivity {
    ...
    private RecyclerView rvJodiSuggestions;
    ...
    private void initJodiSuggestions() {
        ...
        inD.addTextChangedListener(new android.text.TextWatcher() {
            @Override
    ...
    public void proceed(View view) {
        ...
        String pointsInput = inputCoins.getText().toString().trim();
        ...
        if (selected != null && !selected.isEmpty()) {
            ...
            inD.setText("");
            inputCoins.setText("");
            if (TuranPro == 14) findViewById(R.id.llJodiSuggestions).setVisibility(View.GONE);
            return;
        }
    ...
    private void conformDialog(DesawarBPActivity activity, String serverData) {
        submitBidsRecursively(0);
    }

    private void submitBidsRecursively(final int index) {
        if (index >= desawarBidArrayList.size()) {
            // All bids submitted successfully
            progressBar.setVisibility(View.GONE);
            SharPrefClass.setUserCoins(DesawarBPActivity.this, coins.getTitle().toString());
            currentCoins = Integer.parseInt(coins.getTitle().toString()); // Update local balance
            totalCoins = 0; // Reset session total
            mtotalCoins.setText("Total Points : "+ totalCoins); // Reset UI total
        ...
        // Prepare single bid data for the server
        List<DataDesawarBid> singleBidList = new ArrayList<>();
        singleBidList.add(desawarBidArrayList.get(index));
        String gsonData = new Gson().toJson(singleBidList);
        String singleServerData = getString(R.string.bids_api_open) + gsonData + getString(R.string.bids_api_close);

        Call<DataMain> call = ApiClass.getClient().galiDesawarPlaceBid(SharPrefClass.getLoginInToken(DesawarBPActivity.this), singleServerData);
            ...
            public void onResponse(@NonNull Call<DataMain> call, @NonNull Response<DataMain> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (mainModel.getCode().equalsIgnoreCase("505")) {
                        SharPrefClass.setCleaninfo(DesawarBPActivity.this);
                        Toast.makeText(DesawarBPActivity.this, mainModel.getMessage(), Toast.LENGTH_SHORT).show();
                        Intent intent = new Intent(DesawarBPActivity.this, SignInActivity.class);
                        startActivity(intent);
                        finish();
                        return;
                    }
                    if (mainModel.getStatus().equalsIgnoreCase("success")) {
                        // Current bid success, proceed to next
                        submitBidsRecursively(index + 1);
                    } else {
                        progressBar.setVisibility(View.GONE);
                        Toast.makeText(DesawarBPActivity.this, "Failed at bid " + (index+1) + ": " + mainModel.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                } else {
                    progressBar.setVisibility(View.GONE);
