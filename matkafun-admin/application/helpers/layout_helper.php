				...
				<div class="breadcrumb-four" aria-label="breadcrumb">
					<ul class="breadcrumb">
						...
						<li class="active">
							...
							if(!array_key_exists($icon, ICONS)) $icon = 'misc';
...
function text_input(string $NameContainer = null, bool $Require, string $OldValue = null, string $Placeholder, $type = 'text')
{ ?>
	<div class="form-group mb-4">
		<input type="<?= $type ?>" class="form-control <?php echo $NameContainer; ?>" id="<?php echo $NameContainer; ?>" name="<?php echo $NameContainer; ?>" placeholder="<?php echo $Placeholder; ?> <?php $Require === true and print('*'); ?>" <?php $Require === true and print('required'); ?> value="<?php echo $OldValue; ?>">
		<?php if ($Require === true) : ?>
...
{
?>
	<div class="modal fade" id="deleteConfirmModel" tabindex="-1" role="dialog" aria-labelledby="deleteConfirmModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title centre" id="deleteConfirmModalLabel">Are you sure to delete </h5>
...
function sidebar_menu($menus)
{
	foreach ($menus as $key => $menu) :

		if (!isset($menu['sub_menu'])) : ?>
			<li class="menu mt-2">
				<a href="<?php echo SITE_URL . $menu['url']; ?>" aria-expanded="false" class="dropdown-toggle">
					<div class="">
						<?php echo ICONS[$menu['icon']]; ?>
		...
		<?php else : ?>
			<li class="menu mt-2">
				<a href="#<?= ucfirst($key) ?>" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle">
					<div class="">
						<?php echo ICONS[$menu['icon']]; ?>
				...
				<ul class="collapse submenu list-unstyled" id="<?= ucfirst($key) ?>" data-parent="#mainMenu">
					<?php foreach ($menu['sub_menu'] as $sub_menu_key => $sub_menu_value) :
					?>
						<?php if (user_can($sub_menu_value['user_can'])) : ?>
							<li>
								<a href="<?php echo SITE_URL . $sub_menu_value['url']; ?>"><?=ucwords(str_replace('_',' ',$sub_menu_key));?></a>
							</li>
						<?php endif; ?>
					<?php endforeach; ?>
				</ul>
